 ...
 *
 * V1 user upgrading to V1.5 must `pnpm add @xeko-git-1/paykit-stripe @xeko-git-1/paykit-sepay`.
...
export async function resolveProvidersToAdapters(
  ...
  if (legacy.stripe !== undefined) {
    ...
    try {
      const mod = (await import("@xeko-git-1/paykit-stripe")) as {
  ...
  if (legacy.sepay !== undefined) {
    ...
    try {
      const mod = (await import("@xeko-git-1/paykit-sepay")) as {
        createSepayAdapter: (cfg: unknown) => PaymentProviderAdapter;
      };
      createSepayAdapter = mod.createSepayAdapter;
    } catch {
      throw new Error(
        "Legacy `providers: { sepay }` shape requires `pnpm add @xeko-git-1/paykit-sepay`. " +
