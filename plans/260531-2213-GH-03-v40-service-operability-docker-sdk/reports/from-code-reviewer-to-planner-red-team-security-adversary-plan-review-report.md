...
- **Flaw:** The plan proposes `openapi-fetch` as the runtime client (`createPaykitClient` middleware). `openapi-fetch` becomes a RUNTIME dependency of the published `@vibecc/paykit-sdk`, pulling its transitive tree into every downstream payment integrator. The plan only gates on "peer-compat with OpenAPI 3.1" and never addresses version pinning (project rule: pin exact versions) or supply-chain review. A runtime dep in a payment-handling SDK widens the blast radius of any future compromise of that package.
